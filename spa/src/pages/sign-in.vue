...
const toast = useToast();
    ...
    const sendSignInLink = Actor.act(async ()=>{
        try {
            ...
            handleErrorInUI(toast, e, "Failed to send link.", "Something went wrong while trying to send your sign in link (probably on our side). Please try again and get in touch if this error persists.");
        }
    });

    const signInWithThirdPartyAccount = (provider: Parameters<typeof userAuthentication.signInWithThirdPartyAccount>[0]) => Actor.act(async ()=>{
        try {
            await userAuthentication.signInWithThirdPartyAccount(provider);
        } catch (e) {
            handleErrorInUI(toast, e, "Failed to sign in.", "Something went wrong while trying to sign in with your third-party account account. Please try again and get in touch if this error persists.");
        }
    });
        ...
        await actor.act([
            ...
            async ()=>{
                try {
                    ...
                    handleErrorInUI(toast, e, "Failed to sign in.", "Something went wrong while trying to sign in with link. Please try again and get in touch if this error persists.");
                }
            },
    ...
    <SingleCardLayout>
        ...
        <template #default>
            <div class="flex flex-col gap-4">
                <Logo class="mx-auto" />
                <p class="text-center max-w-96 mx-auto px-4 my-4">Please enter your email address and we'll send you a sign in link.</p>
        ...
        <template #footer>
            <Button class="w-full" :disabled="!emailAddressValid" @click="sendSignInLink">Get Link</Button>
            <Divider />
            <p class="text-center my-8">Or sign in with...</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Button class="w-full" severity="secondary" @click="signInWithThirdPartyAccount('google')()">
                    <Vector name="google" class="size-5 fill-white dark:fill-surface-900" />
                </Button>
                <Button class="w-full" severity="secondary" @click="signInWithThirdPartyAccount('github')()">
                    <Vector name="github" class="size-6 fill-white dark:fill-surface-900" />
                </Button>
                <Button class="w-full" severity="secondary" @click="signInWithThirdPartyAccount('twitter')()">
                    <Vector name="twitter" class="size-6 fill-white dark:fill-surface-900" />
                </Button>
                <Button class="w-full" severity="secondary" @click="signInWithThirdPartyAccount('facebook')()">
