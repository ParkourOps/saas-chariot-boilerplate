...
>(
    apiCall: ReturnType<
        typeof apiCallDeclarator.declare<CallName, CallDescription, CallRequiresAuth, CallRequestDataShape, CallRequestDataUnknownKeys, CallRequestDataCatchAll, CallResponseDataShape, CallResponseDataUnknownKeys, CallResponseDataCatchAll>
    >,
    callOptions?: HttpsCallableOptions,
) {
    return async function (requestData: objectOutputType<CallRequestDataShape, CallRequestDataCatchAll, CallRequestDataUnknownKeys>) {
        const rawSuccessResponse = (await httpsCallable(functions, apiCall.name, callOptions)(requestData)).data;
